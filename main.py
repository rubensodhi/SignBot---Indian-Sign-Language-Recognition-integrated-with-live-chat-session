...
from collect_imgs import collect_db
# from Prediction import pred_main
from inference_classifier import Inference_main
from create_dataset import db_onehand
from create_dataset_twohand import db_twohand
    ...
    while True:
        ...
        if results:
            ...
            for i in results:
                ...
                label3 = Label(f1, text="User Panel", font=("arial", 20, "bold"), bg="CadetBlue3", fg="white",
                               relief=SUNKEN)
                label3.pack(side=TOP, fill=X)

                label4 = Label(f2, text="                            Indian Sign Language Recognition System", font=("arial", 10, "bold"), bg="CadetBlue3",
                               fg="white")
                label4.pack(side=BOTTOM, fill=X)
                statusbar = Label(f1, text="                            Indian Sign Language Recognition System", font=("arial", 8, "bold"),
                                  bg="CadetBlue3", fg="white", relief=SUNKEN, anchor=W)
                ...
                class AnimatedGIF(Label, object):
                    def __init__(self, master, path, forever=True):
                        ...
                        try:
                            self._delay = im.info['duration':500]
                        except:
                            self._delay = 30
                ...
                label4 = Label(f3, text="                            Indian Sign Language Recognition System", font=("arial", 10, "bold"), bg="CadetBlue3",
                               fg="white")
                label4.pack(side=BOTTOM, fill=X)

                # =========================Main Buttons=========================================

                btn2w2 = ttk.Button(f1, text="Create Sign", command=collect_db)
                btn2w2.place(x=255, y=100, width=150, height=30)

                btn3w2 = ttk.Button(f1, text="Create DS One Hand", command=db_onehand)
                btn3w2.place(x=255, y=140, width=150, height=30)

                btn10w2 = ttk.Button(f1, text = "Create DS Two Hand", command=db_twohand)
                btn10w2.place(x=255, y=180, width=150, height=30)

                btn6w2 = ttk.Button(f1, text="Predict Signs", command=callThreads)
                btn6w2.place(x=255, y=220, width=150, height=30)

                # =========================Developers Page=========================================

                label10 = Label(f4, text="", font=("arial", 20, "bold"), bg="CadetBlue3", fg="white")
                label10.pack(side=TOP, fill=X)
                label11 = Label(f4, text="     Indian Sign Language Recognition System", font=("arial", 10, "bold"), bg="CadetBlue3",
                                fg="white")
                label11.pack(side=BOTTOM, fill=X)

                label10 = Label(f4, text=" Developed by Ruben Singh Sodhi and Anubhav Gupta", font=("arial", 12, "bold"))
                label10.place(x=50, y=150)
...
if __name__ == "__main__":
    ...
    Label1 = Label(window, text="Login Panel", font=("arial", 20, "bold"), bg="CadetBlue3", fg="white")
    Label1.pack(side=TOP, fill=X)
    Label2 = Label(window, text="", font=("arial", 10, "bold"), bg="CadetBlue3", fg="white")
